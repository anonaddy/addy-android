...
class CreateRuleActivity : BaseActivity(), ConditionBottomDialogFragment.AddConditionBottomDialogListener,
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        InsetUtils.applyBottomInset(binding.activityRulesCreateLL1)
        ...
        if (json?.isNotEmpty() == true && recipientsJson?.isNotEmpty() == true) {
            ...
            if (recipientsStringFromBundle.isNullOrEmpty() || recipientsStringFromBundle == "null") {
                ...
                val recipientsType = object : TypeToken<ArrayList<Recipients>>() {}.type
                val recipientsList = GsonTools.gson.fromJson<ArrayList<Recipients>>(recipientsStringFromBundle, recipientsType)
                recipients = recipientsList
                loadRule(b)
            }
        }
    ...
    private fun setOnClickListeners() {
        toolbarSetAction(binding.activityRulesToolbar, R.drawable.ic_check) {
            ...
            if (ruleId != null) {
                // Update the rule
                lifecycleScope.launch {
                    when (val result = viewModel.updateRule(ruleId!!, rules)) {
                ...
                // Post the rule
                lifecycleScope.launch {
                    when (val result = viewModel.createRule(rules)) {
                        is NetworkResult.Success -> {
                            shouldRefreshOnFinish = true
                            finish()
                        }
    ...
    private suspend fun getAllRecipients(b: Bundle?) {
        val result = viewModel.getAllRecipients()
        if (result is NetworkResult.Success) {
            recipients = ArrayList(result.data.data)
            loadRule(b)
        } else {
    ...
    private suspend fun getRuleInfo(id: String) {
        val result = viewModel.getSpecificRule(id)
        if (result is NetworkResult.Success) {
            /**
             *  CREATE RULE
             */
