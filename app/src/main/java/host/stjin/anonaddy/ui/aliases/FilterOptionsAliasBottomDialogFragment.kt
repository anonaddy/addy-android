...
class FilterOptionsAliasBottomDialogFragment : BaseBottomSheetDialogFragment(), View.OnClickListener {
    private val viewModel: AliasesViewModel by activityViewModels()
    ...
    private var listener: AddFilterOptionsAliasBottomDialogListener? = null
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        arguments?.getString(ARG_ALIAS_SORT_FILTER)?.let { json ->
            val parsed = GsonTools.jsonToAliasSortFilterObject(requireContext(), json)
            if (parsed != null) {
                aliasSortFilter = parsed
            }
        }
    }
    ...
    override fun onCreateView(
        ...
        listener = (parentFragment as? AddFilterOptionsAliasBottomDialogListener) ?: (activity as? AddFilterOptionsAliasBottomDialogListener)
    ...
    private fun disableWatchedOption() {
        val aliasesToWatch = viewModel.getAliasesToWatch().toList()
    ...
    companion object {
        private const val ARG_ALIAS_SORT_FILTER = "arg_alias_sort_filter"

        fun newInstance(
            aliasSortFilter: AliasSortFilter
        ): FilterOptionsAliasBottomDialogFragment {
            return FilterOptionsAliasBottomDialogFragment().apply {
                arguments = Bundle().apply {
                    putString(ARG_ALIAS_SORT_FILTER, GsonTools.gson.toJson(aliasSortFilter))
                }
            }
        }
    }
