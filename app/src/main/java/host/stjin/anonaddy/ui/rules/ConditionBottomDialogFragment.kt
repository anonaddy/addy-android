...
class ConditionBottomDialogFragment(private val conditionEditIndex: Int?, private val conditionEditObject: Condition?) :
    ...
    private fun updateUi(context: Context) {
        if (conditionEditObject != null) {
            val typeIndex = conditionTypes.indexOf(conditionEditObject.type)
            if (typeIndex != -1) {
                binding.bsRuleConditionTypeMact.setText(conditionTypeNames[typeIndex], false)
            }
            updateMatchAndValuesForSelectedType(context, conditionEditObject.match, conditionEditObject.values)
